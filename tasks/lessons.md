...
### 後処理で「区画」を消すなら関連配列も一緒に見る(Codex 検出, #9)
- 駅エリアで plots を除去しても sideRoads は残り、側道+停止線+カーブミラーが駅広場を
  横切っていた(seed=4 で再現)。レイアウトの後処理除去は「その帯を参照する全配列」
  (plots / sideRoads / poles)を列挙して一括で扱う。
- 後処理ブロック同士(駅→ランドマーク)の実行順にも相互作用がある: 先に走った側の成果物
  (駅前商店)を後段(landmark 束ね)が壊せる(seed=7×school で再現)。保護フラグ
  (stationFront)を立てて後段のフィルタで除外する方式が最小修正。
- fallback 経路も本経路と同じ制約検証(帯回避)を通す。「最終手段だから雑でいい」は
  Codex に具体 seed 付きで刺される。決定的スキャン(rng 不使用)なら再現性も保てる。

### 2.5D 地形化(段状テラス)の設計原則(#10 坂の街, 2026-07-12)
- **段関数 hillYAt(isHill,z)=floor((z+L/2)/T)*S を全経路の高さ源にする**。非 hillside は 0 を返す
  ので既存ロケーションは乱数・レイアウトとも完全不変(指紋ハッシュ master 一致で実証)。
- **テラス境界を bands 化して区画を段内へ整列**(258 区画/5 seed で段またぎ 0)。これで区画=
  単一段になり、剛体移動で段に載せても建物がちぎれない/食い込まない。
- **区画は剛体移動(placePlot ラッパ)で段に載せる**。頂点ごとの地形変位は縦面をせん断で歪める
  ので厳禁。footprint 山側(最高段)= groundY に載せ、谷側から適応スカートで埋めると段またぎ
  ランドマーク(26m>21m)も浮き/食い込みゼロ。W.gi は append オフセットで補正(夜窓前提維持)。
- **カメラ地形追従は cam.pos.y を「地面からの高さ(eyeH)」に保ち、描画時に +hillYAt(cam.z)**。
  hash は eyeH を保存するので URL 往復が整合する(world-y を保存すると二重加算で壊れる)。
- **落下 FX(雨・花びら)はカメラ z 一律でなく粒子ごとの z で段に載せる**(Codex 指摘)。
  カメラ ±26m の粒子は隣接段へまたぐので、一律 base だと境界で浮き/埋没する。
- **clamp する高さ関数は、実ジオメトリも同じ範囲までカバーする**。hillYAt は範囲外を端段へ
  clamp するが buildHillside が ±105 で止まると、カメラ(±140)や遠景ビルが接地先を失う。
  街外にも clamp 高さの平地を敷く。

### git: feature ブランチ作成を忘れて master に直接コミットした時の復旧(#10)
- `git reset --hard` は current(default)ブランチだと auto mode に正しくブロックされる。
...
## 2026-07-19 — 09-flow-field（フローフィールド + 図地反転の断念）

### 点描レンダリングでは「図地反転」は立たない
- 1粒=1ドットの加算描画は、物理的に分布が偏っても「ベタに光る面」と「真っ黒な地」を
  作れない。粒・リング・迷子のノイズが常に図を溶かす。面の反転を見せたいなら
  密度場→閾値の可視化層（スライム構成）が必要で、それでも力学の平衡設計が別途重い。
- 6 アプローチ（方向反転/勾配降下/密度散布/ポテンシャル+クランプ/潮汐+閾値レンダ）を
  試して断念。詳細は docs/260719/STATUS.md。

### 弱い統計で「成功」と言い張らない
- corr −0.16 を「反転しています」と報告してユーザーに「見えないよ」と正される失態。
  知覚で見えない効果は失敗。数値は補助であり、スクリーンショットを自分の目で
  判定してから報告する。

### パラメータ堂々巡りは構成が間違っているサイン
- 同じ目標に対しパラメータ調整を 3 回失敗したら、力学・レンダリング・検証指標の
  どれかの「構成」を疑う。8 回回して初めて止まったのは遅すぎた。

### 減衰系粒子の密度は流速に反比例する
- ダンピングのある粒子は流れの遅い場所に溜まる（密度 ∝ 1/速度）。ノイズ場の
  角度流で流線が見えるのはこの効果。逆に均一に満たしたいときは速度を正規化する。

### 明るい実写素材を暗い演出に馴染ませる処理チェーン
- 水中素材のくすみの正体は「黒浮き（ヘイズ）」。彩度を上げる前に黒点クラッシュ
  `(v-0.12)/0.88` で膜を取る。黄色かぶりは `max(0, min(r,g)-b)` を R,G から引くと
  青・シアン・白を傷めず除去できる。順序: ぼかし→デヘイズ→色かぶり除去→彩度→ガンマ→ベール。

### Playwright での時間依存検証
- フェーズ A/B 比較は navigate 直後から sleep で時刻を狙い、`?v=N` キャッシュバスター
  必須。`sleep N` 単発はブロックされるので `sleep N && echo done` を run_in_background で。
- 動画 autoplay はヘッドレスでブロックされる。`videoEl.play()` を evaluate で叩く
  3 段構え（loadeddata + readyState + click once）をページ側に入れておく。

## 2026-07-22 — 09-flow-field（暖色アクセント + 3重ランダムウォーク）

### 加算ブレンドで暖色アクセントを出すにはランダム割当でなく速度ベース
- 加算ブレンド (`AdditiveBlending`) では青粒子85% + 暖色粒子15%のランダム割当は
  無意味。流線の密集部で青+アンバー→白に飲まれる（15%の暖色が85%の青に物量負け）。
- `pow(random, 3)` で偏らせても、bimodal(25%×0.5-1.0)にしても、アルファ/サイズを
  ブーストしても効果なし。加算の本質が「光の足し算」なので、少数の異色は多数の
  同色に色相ごと洗い流される。
- **正解: 速度→暖色** (`warmth = pow(min(spd*1.0, 1.0), 1.3)`)。速い流線は全粒子が
  暖色になるので加算でも金色として発色する。サージ（脈動）と連動して自然に呼吸。
  遅い散乱粒子は青のまま残る。物理的にも直感的（速度→エネルギー→発熱色）。
- **カーブ調整**: 線形(`min(spd*1.2, 1.0)`) → 全面金色。`pow(..., 2.5)` → ほぼ青。
  `pow(..., 1.3)` が「サージ時に金、凪で青」の良いバランス。5段階の反復で収束。

### ランダムウォークの速度自体もランダムにする
- 一定速度のランダムウォーク (TONE_SPEED=0.05固定) はユーザーが「速度もランダムに
  しよう」と提案。ターゲット到達時に `toneSpeed = 0.02 + random * 0.08` で速度も
  毎回変える → 速い変化と遅い変化が混ざり有機的なリズムに。
- 同じ原理を動画アルファのランダムウォークにも適用（vaSpeed 0.015〜0.075）。

### 暖色の足し算は背景を暗くして引き立てる
- 金色粒子を足すと全体の加算光量が増える。「背景も暗くして金が引き立つように」
  というユーザー判断で uDark の Max を 1.4→2.0 に拡張。
- `mix(1.0, 0.45, uDark)` は uDark>1.82 で負になるので `max(..., 0.02)` でクランプ。
- 同時に動画アルファ（0.15〜1.0）の独立ランダムウォークを新設し、暗い背景フェーズで
  プロシージャル深海が浮き出る → 動画とプロシージャルが入れ替わる新しい表情に。

## 2026-07-23 — 10-neon-tunnel-rush（ビートシンクトンネル・制作中）

### ビート応答は全拍同一にしない（予測可能=退屈）
- キック=FOV パンチ、裏打ち=バウンス、を毎拍同じ強さで入れたら「ルールとして
  予測できてしまって面白くない」とユーザー指摘。08 の「見た目の足し算 NG」と同型の
  教訓で、**応答の一様性は数拍で読まれて飽きる**。
- 正解: **音楽的階層 + 確率ゲート**。マクロは構造（小節頭は強い/4小節ごとの
  ターンアラウンドは一撃/セクション最終小節はビルドアップで密度増）、ミクロは
  揺らぎ（通常拍の重み 0.4〜0.65 のランダム、グリッチは 22%/50%/75% の確率発火、
  裏打ちバウンスは 40% 発火・ブレイクダウン中のみ 90%）。
- VJ の手癖と同じで「必ず反応する」より「たまに強く反応する」方が視線を掴む。

### ブラウザ自動再生制限下のオートスタート設計
- ロード時に Tone.start() を先行試行すると、ジェスチャ前は promise が**保留のまま**
  残る。このとき進行フラグ（unlocking）を握り続ける実装にすると、ユーザーの
  クリック時の再試行が門前払いされ**永久に無音**になる（実地でハマった）。
- 正解: ガードは「解錠成功済みか」だけにして、クリック/キーごとに独立して
  Tone.start() を投げる。多重解決は await 後の同期区間ガードで安全に潰れる。

### 音源非依存 BeatClock（uBeat 供給源の抽象化）
...
## 2026-07-24 — 11 Kaleido Overdrive（伝統色万華鏡 + サイケトランス）

### 動く色面をフィードバックの max 合成に入れると灰色に平均化する
- 万華鏡セルの色面(ステンドグラス)を fb ループ内で合成したら、回転・ズームで
  ピクセルごとに色が入れ替わり続け、**チャネル別 max が全部飽和して灰色**になった。
- 正解: **面はフィードバックの外(ポストパス)で下敷き合成**。トレイルは線だけに。
  面はポストの歪み(渦・モザイク)の後の uv で描くと背景ごと歪んで一体感も出る。

### ワンショットの尻尾は「鳴らす側」でチョークする(重なり=道路工事)
- 1.3〜1.8s 鳴るガバキックを 0.36s 間隔で重ねると 3〜4 発の歪んだ尻尾が積層して
  連続轟音になる。**新しい打が前の打を 10ms リリースで止める**(ハット・ピアノ・
  キック全部同じ型)。「道路工事」の第 1 犯はキックでなく**常時 16 分のローリング
  ベースのアタック**だったのも教訓 — 犯人特定は「常時鳴っている音」から疑う。

### 口笛と審判笛は物理が違う(合成の限界は早めに見切って SFX 生成へ)
- 三角波+ゆっくりビブラート=口笛。審判笛の正体は**豆のコロコロ(30Hz 級の高速 AM)
  + 息のノイズ + 甲高い矩形波**。それでも「やさしい」と言われたら合成に固執せず
  **ElevenLabs 効果音生成**(WebUI)で本物を作る方が速い。再生レートの広域
  ランダム(0.9〜1.6)で「毎回別の笛」になり反復感が消える。

### ジャンルの正体はテンポに宿る(ガバ 160+)
- 145BPM で歪んだキックを鳴らしても「ハードテクノ」。**ガバに聞こえる BPM 帯
  (160-200)に上げる**のが正道で、遅い BPM のままならダブルタイム(裏 8 分)で
  体感密度を作る代替がある(今回は 165 に上げてダブルは撤去)。

### Playwright での再生停止 UI の自動テストは click を使わない
- このページ環境では合成 click が二重配信され、さらに**デスクトップに見えている
  Playwright 窓へのユーザーの実クリック**が pause を発火して観測が汚れる。
  トグル類の検証は **`window.__state` の直接操作**で行い、click 経路は実ブラウザ
  (Comet)の人間確認に任せる。

### 保存済み API キーは失効している前提で疎通確認から入る
- ~/.claude.json の ElevenLabs キーが 401 失効していた。キーを使う自動化は
  **/v1/user 等の軽い疎通 → 本命呼び出し**の順で。失効時はユーザーの WebUI 生成に
  切り替えると速い(キーはスクリプト内でのみ抽出し、ログ・出力に出さない)。
